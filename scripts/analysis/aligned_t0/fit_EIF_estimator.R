...
eif_estimator <- function(df, models, n_splits) {
  ...
  tau_hat <-
    ...
    mutate('alpha_OS' = A * (1 - R/eta1_hat) * epsilon1_star_hat + A * R * eligible/eta1_hat,
           'beta_OS' = 
             A * (epsilon1_star_hat * pct_wt_change - xi_hat)
           + A * R/eta1_hat * ((eligible - epsilon1_star_hat) * pct_wt_change - (eligible * mu0_hat - xi_hat))
           - (1-A) * R/eta1_hat * (eligible * u_hat/(1-u_hat) * (pct_wt_change - mu0_hat) - (gamma_tilde_hat * pct_wt_change - chi_tilde_hat))
           - (1-A) * eta0_hat/eta1_hat * ((gamma_tilde_hat * pct_wt_change - chi_tilde_hat))
    ) %>% 
...
model_list <- 
  list(
    ...
    'epsilon_star' = list('type' = 'super_learner',
                          'Y' = 'eligible',
                          'X' = c('site', 'gender', 'baseline_age', 'smoking_status', 'eGFR', 'race', 'hypertension', 'dyslipidemia', 'calendar_year', 'A', 'pct_wt_change'),
                          'formula' = ~site + gender + baseline_age + smoking_status + eGFR + race + hypertension + dyslipidemia + calendar_year + A + pct_wt_change,
                          'sl_libs' = c('SL.glm', 'SL.gam')),
    ...
    'xi' = list('type' = 'super_learner',
                'Y' = 'Emu0',
                'X' = c('site', 'gender', 'baseline_age', 'smoking_status', 'eGFR', 'race', 'hypertension', 'dyslipidemia', 'calendar_year', 'pct_wt_change'),
                'formula' = ~site + gender + baseline_age + smoking_status + eGFR + race + hypertension + dyslipidemia + calendar_year + pct_wt_change,
                'sl_libs' = c('SL.polymars', 'SL.lm')),
...
write_csv(df_results, glue('data/application/aligned_t0/weight_change/EIF_{s_id}.csv'))
